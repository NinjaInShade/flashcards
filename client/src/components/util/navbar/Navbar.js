    ...
    <React.Fragment>
      ...
      <NavbarWrapper>
        ...
        <PageLinks style={{ marginRight: "50px" }}>
          ...
          {auth.isAuth && <button onClick={() => setAuth({ ...auth, isAuth: false })}>Sign out</button>}
          {!auth.isAuth && <button>Sign up</button>}
          {!auth.isAuth && <button onClick={() => setAuth({ ...auth, isAuth: true })}>Login</button>}
        </PageLinks>
